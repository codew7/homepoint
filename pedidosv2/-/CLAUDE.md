...
## Recent Changes (2026-08-06b)

### Layout de escritorio en `pedidosv2.html`

La UI está pensada para tablet en vertical, y en un monitor horizontal dejaba media pantalla vacía y obligaba a scrollear para pasar del checklist a los totales. Se agregó un layout alternativo **solo por CSS más tres líneas de JS**; en táctil no cambió nada.

- **El gate es `@media (min-width: 1024px) and (pointer: fine)`**, no solo el ancho: una tablet táctil reporta `coarse` aunque en horizontal supere 1024px, así que nunca entra. `pedidosv2Basic.html` **no** recibió el port.
- **El bloque va al FINAL del `<style>`**, no en la sección RESPONSIVE (donde hay un comentario que apunta a él). Puesto ahí arriba lo pisaban por cascada `.pv-content`, `.pv-col-*`, `.msg-dropdown` y `.pv-fecha`, que se definen más abajo con la misma especificidad. Fue un bug real durante la implementación: el `body` grid aplicaba y las dos columnas no.
- **`body` pasa a grid** (`minmax(0,1fr) auto` × `auto auto minmax(0,1fr)`) y `.footer-controls` ocupa la columna 2 como barra lateral de 320px. La pista `auto` hace que la columna **colapse sola** cuando el JS pone el footer en `display:none` (pedido bloqueado); idem la fila 2 con `.status-bar`. No hizo falta tocar ningún `footer.style.display`.
- **El cajón (`.footer-drawer`) queda siempre abierto**: hay que anular las **dos** reglas (base y `.open`), si no `.open` lo achicaría a 320px. `toggleDrawer`/`openDrawer`/`closeDrawer` y el swipe siguen existiendo, simplemente no se notan.
- **`renderUnificado()` emite dos envoltorios**, `.pv-col-items` (items-header + grupos) y `.pv-col-side` (totales + los dos radio-group), y `.pv-content` pasa a grid de dos columnas con el side en `position: sticky`. Fuera del breakpoint los envoltorios llevan **`display: contents`** y se disuelven: por eso se emiten *justo donde ya estaban esos bloques* y no hace falta reordenar con `order`. Verificado: a 390 / 820 / 1023 px las métricas de layout son idénticas al DOM anterior. Con 5 hijos la colocación del grid es determinista (tres filas completas y las dos columnas juntas en la cuarta).
- **`.item-info` pasa a grid** en escritorio: nombre y código siguen apilados a la izquierda y el bloque de precios se corre a la derecha ocupando las dos filas. Pasar todo a una sola fila flex estrangulaba el nombre a ~160px.
- **`.footer-input`/`.nota-select` necesitan `min-width: 0`**: un `<input type=text>` trae ~177px de ancho intrínseco y, como item flex, `min-width:auto` le impedía encogerse — los campos Tamaño/Peso se salían del borde de la barra.
- **El stepper de cantidad arranca desplegado**: la protección anti-toque de tablet no aplica con mouse (la guarda mide desplazamiento entre `pointerdown` y `click`, que con mouse es ~0).
- **Los bottom-sheets se vuelven diálogos centrados** (pedidos, historial, faltantes, agregar artículo, buscar artículo): `align-items: center`, `border-radius` completo, `overflow: hidden` para clipear las esquinas y `modalIn` en vez de `slideUp`. Los `sticky` internos (`.pp-grupo`, `.articulo-multi`) no se ven afectados porque su contenedor de scroll es la lista, no el panel.
- **`#articuloModal` se acota a 620px de ancho**, pero **solo el panel** — nunca el `<video>` ni `#articuloScannerDiv`, por el cálculo de región de html5-qrcode. Equivale a abrir el modal en un dispositivo más angosto. Si el escaneo fallara en escritorio, revertir ese `max-width` primero.
- **Los menús `.print-menu`/`.msg-dropdown` abren hacia abajo** (`top: calc(100% + 6px)`): abrían hacia arriba porque el footer estaba al pie de la pantalla.

## Recent Changes (2026-08-07)

### "Convertir a Retiro" en el menú Mensaje (`pedidosv2.html`)

El slot `data-msg="3"` del `#msgDropdown`, que decía "Próximamente", ahora convierte el pedido en el **mismo mensaje de WhatsApp que genera el catálogo web para un pedido de retiro** (`construirMensajeWhatsAppRetiro()`, `CatalogoWeb/catalogo.js`) y deja la **nota del pedido en `Retira`**. Sirve para dar de alta por la vía comercial un pedido que ya está en Firebase, sin retipearlo.

- **El destino es fijo**: `WHATSAPP_RETIRO_TEL = '5491121891006'` (+54 9 11 2189-1006), el mismo `WHATSAPP_URL` del catálogo. **No es el teléfono del cliente**, a diferencia de las otras dos opciones del menú.
- **La rama sale antes de los guards de teléfono** de `pvEnviarMensaje()`: sin teléfono el mensaje solo omite la línea `📞` (igual que el original), no debe cortarse con "Sin telefono registrado". Guard propio en su lugar: pedido sin items → toast.
- **No sella ni cierra el pedido.** A diferencia de la Cotización (`tipo === '2'`) no llama a `sellarYCerrarCotizacion()` ni `marcarPagoSolicitado()`: no fija precios ni pide cobro. Lo único que escribe es la nota.
- **`marcarRetira()` va sin `await`**, igual que `sellarYCerrarCotizacion()` en la rama de abajo: el `window.open` tiene que salir en el mismo tick del click o el navegador lo bloquea como popup no solicitado.
- **`NOTA_RETIRA = 'Retira'` no toca el eje de cobro**: normalizada no está en `NOTAS_PAGADO_SET` y no menciona pago, así que `estadoPago()` la deja en `'sin-dato'` y el tramo violeta del riel queda como si no hubiera nota. Único guard: no reescribir si ya decía lo mismo (solo movería `notaFecha`). No se corta por `locked` — un pedido ya cerrado igual puede pasar a retiro.
- **`pvEscribirNota(valor)` centraliza la escritura de la nota** (los tres campos `nota`/`notaFecha`/`lastUpdated` + sincronizar el `<select>`, agregando la `<option>` si no viene de `notas!A:A`). `marcarPagoSolicitado()` se redujo a sus guards + esta llamada; los guards de *qué* corresponde escribir quedan en el llamador, porque dentro del helper no hay forma de saber si sobrescribir sería un retroceso.
- **`pvBuildMensajeRetiro()` es un port fiel**, no una reinterpretación: mismo título, mismo fallback `'Sin nombre'`, sangría de **3 espacios**, el `detallado.pop()` que borra la línea en blanco del último ítem y el modo compacto sobre `WHATSAPP_MSG_MAX_CHARS = 1500`. Lo único que cambia son las fuentes de datos (`pedidoActual.cliente`/`.items` en vez de `datosExtraCliente`/`carrito`). Se mantuvieron template-spread y `[...a, ...b]` en vez de la concatenación habitual del archivo **para que el diff contra `catalogo.js` muestre solo esas líneas**. Verificado con 5 casos (normal, sin teléfono, sin código, sin nombre, >1500 chars): salida idéntica carácter por carácter.
- **El `SEPARADOR` son 17 caracteres U+2500 (`─`)**, no guiones ASCII. Si un editor los normaliza, el mensaje deja de coincidir con el del catálogo sin que se note a simple vista.
- **`pvNormalizarTelefonoAr()` va al revés que la normalización de `pvEnviarMensaje()`**, que está pocas líneas más abajo: esta deja los 10 dígitos nacionales para **imprimir** dentro del texto (port de `normalizarTelefonoAr()` de `CatalogoWeb/catalogo.html`), aquella arma el `54…` internacional porque ahí el número es el **destino** de la URL.
- **No se filtran items por `listo`**: el mensaje describe el pedido, no el avance del armado.
- No se portó a `pedidosv2Basic.html`.
