...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // === IMAGEN FLOTANTE PARA ARTÍCULOS ===
  let imagenFlotante = null;

  function crearImagenFlotante() {
    if (imagenFlotante) return imagenFlotante;
    
    imagenFlotante = document.createElement('img');
    imagenFlotante.className = 'imagen-flotante';
    imagenFlotante.alt = 'Imagen del artículo';
    document.body.appendChild(imagenFlotante);
    
    return imagenFlotante;
  }

  function extraerPrimerLinkImagen(columnAValue) {
    if (!columnAValue || typeof columnAValue !== 'string') return null;
    
    // Debug: mostrar el valor de la columna A
    console.log('Valor columna A:', columnAValue);
    
    const links = columnAValue.split(',');
    const primerLink = links[0]?.trim();
    
    console.log('Primer link extraído:', primerLink);
    
    // Validar que sea una URL válida de imagen
    if (primerLink && (primerLink.includes('.jpg') || primerLink.includes('.jpeg') || primerLink.includes('.png') || primerLink.includes('.gif'))) {
      return primerLink;
    }
  ...
  function mostrarImagenFlotante(e, nombreArticulo) {
    console.log('mostrarImagenFlotante llamada con:', nombreArticulo);
    if (!nombreArticulo || !articulosPorNombre[nombreArticulo]) {
      console.log('Artículo no encontrado en articulosPorNombre');
      return;
    }
    ...
    const linkImagen = extraerPrimerLinkImagen(articulo[1]); // Columna A está en índice 0
    console.log('Link de imagen obtenido:', linkImagen);
    
    if (!linkImagen) {
      console.log('No hay link de imagen válido');
      return;
    }
    
    const img = crearImagenFlotante();
    
    // Posicionar la imagen cerca del cursor
    img.style.left = (e.clientX + 15) + 'px';
    img.style.top = (e.clientY - 75) + 'px';
    img.style.display = 'block';
    
    console.log('Mostrando imagen flotante en posición:', e.clientX + 15, e.clientY - 75);
    
    // Mostrar texto de carga primero
    ...
    tempImg.onerror = function() {
      ...
      img.innerHTML = 'No disponible';
      img.classList.remove('cargando');
    };
    tempImg.src = linkImagen;
  }
  ...
  function ocultarImagenFlotante() {
    if (imagenFlotante) {
      ...
      imagenFlotante.classList.remove('cargando');
    }
  }
...
function getTipoCliente() {
  ...
  function renderItems() {
    ...
    items.forEach((item, idx) => {
      ...
      // === EVENTOS PARA IMAGEN FLOTANTE EN SELECT2 ===
      // Agregar event listeners al contenedor de Select2 para la imagen flotante
      setTimeout(() => {
        const select2Container = $(row).find('.select2-container');
        if (select2Container.length) {
          select2Container.on('mouseenter', function(e) {
            const selectedValue = $select.val();
            if (selectedValue && selectedValue.trim() !== '') {
              mostrarImagenFlotante(e.originalEvent, selectedValue);
            }
          });
          
          select2Container.on('mousemove', function(e) {
            const selectedValue = $select.val();
            if (selectedValue && selectedValue.trim() !== '' && imagenFlotante && imagenFlotante.style.display === 'block') {
              // Actualizar posición de la imagen flotante siguiendo el cursor
              imagenFlotante.style.left = (e.originalEvent.clientX + 15) + 'px';
              imagenFlotante.style.top = (e.originalEvent.clientY - 75) + 'px';
            }
          });
          
          select2Container.on('mouseleave', function() {
            ocultarImagenFlotante();
          });
        }
      }, 100);
    ...
    itemsBody.querySelectorAll('.nombre-select').forEach((select, idx) => {
      select.addEventListener('change', function() {
        ...
        if (cantidadInput) {
          cantidadInput.focus();
          // Mover el cursor al final del valor SOLO si no es tipo number
    ...
    // === EVENTOS PARA IMAGEN FLOTANTE EN SELECTS ===
    itemsBody.querySelectorAll('.nombre-select').forEach((select, idx) => {
      // Agregar event listeners para mostrar imagen flotante cuando hay un artículo seleccionado
      select.addEventListener('mouseenter', function(e) {
        const nombreArticulo = this.value;
        if (nombreArticulo && nombreArticulo.trim() !== '') {
          mostrarImagenFlotante(e, nombreArticulo);
        }
      });
      
      select.addEventListener('mousemove', function(e) {
        const nombreArticulo = this.value;
        if (nombreArticulo && nombreArticulo.trim() !== '' && imagenFlotante && imagenFlotante.style.display === 'block') {
          // Actualizar posición de la imagen flotante siguiendo el cursor
          imagenFlotante.style.left = (e.clientX + 15) + 'px';
          imagenFlotante.style.top = (e.clientY - 75) + 'px';
        }
      });
